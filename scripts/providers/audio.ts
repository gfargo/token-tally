...
import type { AudioPricingRecord } from "../types";
import { getBaselineProvider } from "./baseline";
import { parseOpenAIAudio } from "./openai/parser-multi";
...
const fetchRemoteMarkdown = async (): Promise<string | null> => {
  try {
    ...
    console.warn(
      `[pricing][audio] remote fetch failed: ${
...
const loadFixtureMarkdown = async (): Promise<string | null> => {
  try {
    ...
    if (nodeError.code !== "ENOENT") {
      console.warn(
        `[pricing][audio] failed to read fixture: ${nodeError.message}`
      );
    }
    return null;
  }
...
export const fetchAudio = async (): Promise<AudioPricingRecord> => {
  const baseline = await getBaselineProvider("audio");
  ...
  if (!useFixtureOnly) {
    ...
    if (markdown && !markdown.includes("### Transcription and speech generation")) {
      console.warn(
        "[pricing][audio] remote response missing expected Transcription section; falling back to fixture"
      );
      markdown = null;
    }
    if (markdown) {
  ...
  if (!markdown) {
    console.warn(
      "[pricing][audio] falling back to baseline pricing due to missing markdown source"
    );
    return baseline;
  }

  try {
    const parsed = parseOpenAIAudio(markdown);
    return Object.keys(parsed).length ? parsed : baseline;
  } catch (error) {
    console.warn(
      `[pricing][audio] parse failure from ${
        source ?? "unknown"
    ...
    if (source === "remote") {
      try {
        ...
        if (fixtureMarkdown) {
          const parsed = parseOpenAIAudio(fixtureMarkdown);
          return Object.keys(parsed).length ? parsed : baseline;
        }
      } catch (fixtureError) {
        console.warn(
          `[pricing][audio] fixture parse failure after remote error: ${
    ...
    console.warn("[pricing][audio] falling back to baseline pricing data");
    return baseline;
  }
};
